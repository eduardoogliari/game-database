...
module.exports = function (pool) {
    ...
    router.get("/", (req, res) => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const nomeJogo = (_e = req.query.nome) === null || _e === void 0 ? void 0 : _e.toString();
            const generoJogo = (_f = req.query.genero) === null || _f === void 0 ? void 0 : _f.toString();
            const plataformaJogo = (_g = req.query.plataforma) === null || _g === void 0 ? void 0 : _g.toString();
            const devJogo = (_h = req.query.desenvolvedora) === null || _h === void 0 ? void 0 : _h.toString();
            const pubJogo = (_j = req.query.publicadora) === null || _j === void 0 ? void 0 : _j.toString();
            ...
            const sqlQuery = `SELECT DISTINCT id, nome, imagem_capa_url, data_lancamento, descricao FROM jogo \
                ...
                ${devQuery} \
                ${whereQuery}`;
            const contagemResult = yield pool.query(`SELECT COUNT(*) AS contagem FROM (${sqlQuery}) AS q`);
            ...
            const result = yield pool.query(`${sqlQuery} ORDER BY ${sortAttribute} ${sortOrder} LIMIT $1 OFFSET $2`, [limitOption, offset]);
            let jogosArray = [];
    ...
    router.get("/:jogoId", (req, res) => __awaiter(this, void 0, void 0, function* () {
        try {
            const id = req.params.jogoId;
            const result = yield pool.query("SELECT id, nome, imagem_capa_url, data_lancamento, descricao FROM jogo WHERE jogo.id=$1", [id]);
            let jogo = {
                jogoId: undefined,
                jogoNome: '',
                dataLancamento: '',
            ...
            if (result.rowCount && result.rowCount > 0) {
                ...
                const genResult = yield pool.query(`SELECT genero_id AS id, gen.gen_nome AS nome FROM jogo INNER JOIN jogo_generos ON jogo_generos.jogo_id=jogo.id INNER JOIN (SELECT genero.nome AS gen_nome, genero.id AS gen_id FROM genero) AS gen ON gen.gen_id=jogo_generos.genero_id WHERE jogo_id=$1;`, [jogoId]);
                const plataformaResult = yield pool.query(`SELECT jogo_plataformas.plataforma_id AS id, plat.plat_nome AS nome FROM jogo INNER JOIN jogo_plataformas ON jogo.id=jogo_plataformas.jogo_id INNER JOIN (SELECT plataforma.nome_popular AS plat_nome, plataforma.id AS plat_id FROM plataforma) AS plat ON plat.plat_id=jogo_plataformas.plataforma_id WHERE jogo_id=$1;`, [jogoId]);
                const imagensResult = yield pool.query(`SELECT jogo_imagens.imagem_url as url, jogo_imagens.legenda FROM jogo_imagens WHERE jogo_id=$1;`, [jogoId]);
                let imagens = [];
